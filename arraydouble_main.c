...
int
processFasta(char *filename, FASTArecord **records, int *allocatedSize,
			 double *timeTaken)
{
	FILE *fp;
	FASTArecord fRecord;
	...
	do {
		...
		* we know something is happening */
		...
		if (status == 0) {
			...
			if (recordNumber >= *allocatedSize){
				
                newAllocatedSize = (*allocatedSize == 0) ? 1 : (*allocatedSize * 2);
                FASTArecord *newRecords = realloc(*records, newAllocatedSize * sizeof(FASTArecord));
	...
	double waste = 100 - ((double) recordNumber / (double) *allocatedSize * 100);
	...
	)
{
	...
	for (i = 0; i < repeatsRequested; i++) {
		int allocatedSize = 10000;
    	FASTArecord *records = (FASTArecord *)malloc(allocatedSize * sizeof(FASTArecord));
		status = processFasta(filename, &records, &allocatedSize, &timeThisIterationInSeconds);
		free(records);
	...
	for (i = 1; i < argc; i++) {
		if (argv[i][0] == '-') {
			if (argv[i][1] == 'R') {
				...
				if (repeatsRequested < 1){
					fprintf(stderr,
						"Error: invalid number of repeats\n");
					return 1;
				}
			} else {
				fprintf(stderr,
