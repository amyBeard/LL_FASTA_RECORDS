...
int
processFasta(char *filename, double *timeTaken, LinkedList *ll)
{
	FILE *fp;
	FASTArecord fRecord;
	...
	do {
		...
		* we know something is happening */
		...
		if (status == 0) {
			...
			recordNumber++;
			if (recordNumber==1){
				ll->head->key= strdup(fRecord.description);
				ll->head->value = (void*) fRecord.id;
				ll->tail = ll->head;
			}else{
	...
	)
{
	...
	for (i = 0; i < repeatsRequested; i++) {
		LinkedList ll;
		ll.head = llNewNode(NULL, 0);
		ll.tail = ll.head;
		status = processFasta(filename, &timeThisIterationInSeconds, &ll);
		if (status < 0)	return -1;
		totalTimeInSeconds += timeThisIterationInSeconds;
		llFree(ll.head, deleteKey, NULL);
	}
