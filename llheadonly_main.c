...
int
processFasta(char *filename, double *timeTaken, LLvNode * node)
{
	FILE *fp;
	FASTArecord fRecord;
	...
	do {
		...
		* we know something is happening */
		...
		if (status == 0) {
			...
			recordNumber++;
			if (recordNumber==1){
				node->key = strdup(fRecord.description);
				node->value = (void*) fRecord.id;
			}else{
				LLvNode * newNode = llNewNode(strdup(fRecord.description),  (void *)fRecord.id);
				if (newNode != NULL){
					node = llAppend(node, newNode);
				}else{
					printf("Error: couldn't add node to LL\n");
				}
	...
	)
{
	...
	for (i = 0; i < repeatsRequested; i++) {
		LLvNode * node = llNewNode(NULL, 0);
		status = processFasta(filename, &timeThisIterationInSeconds, node);
		if (status < 0)	return -1;
		totalTimeInSeconds += timeThisIterationInSeconds;
		llFree(node, deleteKey, NULL);
	}
